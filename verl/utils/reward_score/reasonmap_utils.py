...
def extract_stops_no_quotes(model_answer: str) -> List[str]:
    s = model_answer

    m = re.search(r"Stops\s*:\s*\[", s, flags=re.IGNORECASE)
    if not m:
        return []
...
if __name__ == '__main__':
    answer = ("So, the intermediate stops between Encarnação and S. Sebastião are:"
             "Stops: [Campo Grande, Alvalade, Roma, Areeiro, Olaias, Bela Vista, Chelas, Marvila].\nThere are 8 intermediate stops."
    )

    print(extract_stops_no_quotes(answer))  # Expected: ['Campo Grande', 'Alvalade', 'Roma', 'Areeiro', 'Olaias', 'Bela Vista', 'Chelas', 'Marvila']